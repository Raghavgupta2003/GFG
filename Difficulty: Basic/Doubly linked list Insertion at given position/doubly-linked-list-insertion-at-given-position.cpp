...
class Solution {
  public:
    // Function to insert a new node at given position in doubly linked list.
    void insertathead(Node *head, int pos, int data){
    ...
    // void insertatEnd(Node *head, int pos, int data){
    //     Node* temp = head;
    ...
    //     newNode->prev = temp;
    // }
    Node *insertAtidx(Node *head, int pos, int data) {
        // code here
        Node* temp = head;
        for(int i=0; i<pos;i++){
            temp = temp->next;
    ...
    Node *addNode(Node *head, int pos, int data) {
        // code here
        if(pos == 0) insertathead(head,pos,data);
        else{
            insertAtidx(head,pos,data);
        }
